...
from floodlightapp_consts import *
import floodlight_rest
import floodlight_firewall
import floodlight_sfp
import validators
...
class FloodlightConnector(BaseConnector):
    ...
    def _test_connectivity(self, url, param):
        try:
            ...
            self.set_status(phantom.APP_ERROR, FLOODLIGHTAPP_ERR_SERVER_CONNECTION, e)
            self.append_to_message(FLOODLIGHTAPP_ERR_CONNECTIVITY_TEST)
            return self.get_status()

        return self.set_status_save_progress(phantom.APP_SUCCESS, FLOODLIGHTAPP_SUCC_CONNECTIVITY_TEST)
    ...
    def _unblock_flow(self, url, param):
        ...
        try:
            ...
            msg = result.get("status", FLOODLIGHTAPP_ERR_NO_STATUS)
            if msg == u'Rule added':
    ...
    def _block_address(self, url, param):
        if (validators.validate_ip4(param["ip_macaddress"]) or
            ...
            return self.set_status(phantom.APP_ERROR, FLOODLIGHTAPP_ERR_PARAM)
    ...
    def _delete_firewall_rule(self, url, param):
        ...
        try:
            ...
            self.save_progress("done.")
            if result is None:
                return action_result.set_status(phantom.APP_ERROR, FLOODLIGHTAPP_ERR_NO_DATA)

            action_result.add_data(result)
            if result.get("success") is not True:
                return action_result.set_status(phantom.APP_ERROR, result.get("status"))

            return action_result.set_status(phantom.APP_SUCCESS, FLOODLIGHTAPP_SUCC)
    ...
    def _list_switches(self, url, param):
        ...
        for switch in switch_list:
            ...
            if not addrmatch:
                return action_result.set_status(phantom.APP_ERROR, FLOODLIGHTAPP_ERR_PARSE, switch["inetAddress"])
    ...
    def handle_action(self, param):
        ...
        controller_url = config.get(FLOODLIGHTAPP_CONTROLLER_URL)
